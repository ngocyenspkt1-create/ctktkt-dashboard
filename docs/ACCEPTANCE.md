...
- Đã push commit `2427c27` lên `github/main`; GitHub xác nhận trạng thái Vercel `success` và deployment đã hoàn tất.
- Còn thiếu: chưa ghi dữ liệu production vì phiên công cụ không có trình duyệt đăng nhập. Người dùng đăng nhập, bấm `Nhập lịch sử Mục 1`, chọn gói JSON và chờ thông báo đọc lại thành công. Sau đó có thể tự đồng bộ Mục 2/nhật ký theo yêu cầu.
...
- Web kiểm tra phiên bản tiện ích trước khi đồng bộ. Nếu đang kết nối bản thấp hơn `0.4.27`, nút đổi thành **Reload tiện ích 0.4.27** và không cho bản cũ tiếp tục đồng bộ. Web ưu tiên hướng dẫn Reload đối với tiện ích Load unpacked từ thư mục nguồn cố định; ZIP chỉ là phương án dự phòng khi Chrome/Edge đang trỏ tới thư mục giải nén cũ hoặc thư mục nguồn không còn được cập nhật.
- Kiểm tra sau bổ sung cơ chế cập nhật: **125/125 test đạt**, TypeScript đạt và build production đạt. `storage:check` vẫn bị chặn do máy local thiếu `TURSO_DATABASE_URL`.
