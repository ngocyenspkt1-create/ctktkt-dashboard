...
export function DailyProductionTable() {
  ...
  function syncFromQlkt(){
    ...
    if(!extensionVersion||extensionOutdated){
      const link=document.createElement("a");link.href=QLKT_EXTENSION_DOWNLOAD_URL;link.download=`qlkt-sync-extension-${REQUIRED_QLKT_EXTENSION_VERSION}.zip`;document.body.appendChild(link);link.click();link.remove();
      window.postMessage({channel:"ctktkt-qlkt-sync",sender:"ctktkt-web",type:"PING"},window.location.origin);setSyncHelp(true);
      setError(extensionOutdated?`Tiện ích v${extensionVersion} đã cũ. Web đã tải bản ${REQUIRED_QLKT_EXTENSION_VERSION}; hãy giải nén, nạp lại tiện ích rồi nhấn Ctrl+F5.`:`Chưa kết nối tiện ích QLKT. Web đã tải bản ${REQUIRED_QLKT_EXTENSION_VERSION}; hãy giải nén và cài tiện ích.`);return;
    }
  ...
  return <section className="space-y-3">
    ...
    <DialogPrimitive.Root open={syncHelp} onOpenChange={setSyncHelp}><DialogPrimitive.Portal><DialogPrimitive.Overlay className="fixed inset-0 z-50 bg-black/45"/><DialogPrimitive.Content className="fixed left-1/2 top-1/2 z-50 grid w-[calc(100%-2rem)] max-w-lg -translate-x-1/2 -translate-y-1/2 gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl outline-none"><DialogPrimitive.Title className="text-lg font-bold text-[#173b64]">Cập nhật tiện ích QLKT {REQUIRED_QLKT_EXTENSION_VERSION}</DialogPrimitive.Title><DialogPrimitive.Description className="text-sm leading-6 text-slate-600">Web đã tải gói mới khi bạn nhấn đồng bộ. Do bảo mật của Chrome/Edge, web không thể tự cài đè tiện ích: hãy giải nén ZIP, mở trang quản lý tiện ích, chọn lại thư mục bằng Load unpacked hoặc thay nội dung thư mục cũ rồi nhấn Reload; cuối cùng nhấn Ctrl+F5 tại web.</DialogPrimitive.Description><div className="rounded-xl border border-blue-100 bg-blue-50 p-3 text-sm text-[#274f78]"><p className="font-bold">Tiện ích không đọc hoặc lưu mật khẩu.</p><p className="mt-1">Dữ liệu luôn quay về bảng kiểm tra trước và chỉ được lưu khi bạn xác nhận.</p></div><div className="flex justify-end gap-2"><button type="button" onClick={()=>setSyncHelp(false)} className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700">Đóng</button><a href={QLKT_EXTENSION_DOWNLOAD_URL} download={`qlkt-sync-extension-${REQUIRED_QLKT_EXTENSION_VERSION}.zip`} className="rounded-lg bg-[#334785] px-4 py-2 text-sm font-semibold text-white">Tải lại bản {REQUIRED_QLKT_EXTENSION_VERSION}</a></div></DialogPrimitive.Content></DialogPrimitive.Portal></DialogPrimitive.Root>
