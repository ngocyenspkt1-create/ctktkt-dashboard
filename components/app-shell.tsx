...
// AppShell là Server Component (async) nên tự đọc phiên đăng nhập qua cookie —
// middleware.ts đã đảm bảo mọi trang gọi tới đây đều có người dùng hợp lệ, nên
// không cần các page.tsx truyền user xuống. Bọc children trong SessionProvider
// để các component con (client) biết vai trò hiện tại mà ẩn/khoá nút sửa dữ liệu.
export async function AppShell({ children, active, hideSearch }: { children: ReactNode; active: string; hideSearch?: boolean }) {
  ...
  return <SessionProvider user={user}><main className="flex min-h-screen bg-[#f5f6f8] text-[#17213b]">
    <aside className="sticky top-0 hidden h-screen w-[235px] shrink-0 flex-col border-r border-slate-200 bg-white lg:flex">
      ...
      <nav className="mt-2 flex-1 px-2"><p className="px-3 py-2 text-[10px] font-extrabold tracking-[0.16em] text-slate-400">QUẢN LÝ CHỈ TIÊU</p>{items.map(item => {
