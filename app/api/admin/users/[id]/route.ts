...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    let existing: (UserRow & { position?: string }) | null = null;
    try {
      existing = await db.prepare("SELECT id, role, position FROM users WHERE id = ?").bind(id).first() as (UserRow & { position?: string }) | null;
    } catch {
      existing = await db.prepare("SELECT id, role FROM users WHERE id = ?").bind(id).first() as (UserRow & { position?: string }) | null;
    }
    ...
    try {
      ...
      updated = await db.prepare("SELECT id, username, display_name AS displayName, role, created_at AS createdAt FROM users WHERE id = ?").bind(id).first();
    }
    ...
    return Response.json({
      error: err instanceof Error ? err.message : "Lỗi khi cập nhật tài khoản.",
    }, { status: 500 });
  }
}
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return Response.json({
      error: err instanceof Error ? err.message : "Lỗi khi xóa tài khoản.",
    }, { status: 500 });
  }
}
